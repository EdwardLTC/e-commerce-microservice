...
type OrderItem struct {
	...
	// VariantDescription holds the value of the "variant_description" field.
	...
	order_items  *uuid.UUID
	refund_items *uuid.UUID
...
func (*OrderItem) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case orderitem.FieldProductName, orderitem.FieldVariantDescription, orderitem.FieldImageURL:
			values[i] = new(sql.NullString)
		case orderitem.FieldCreatedAt, orderitem.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case orderitem.FieldID, orderitem.FieldProductID, orderitem.FieldVariantID:
			values[i] = new(uuid.UUID)
		case orderitem.ForeignKeys[0]: // order_items
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
		case orderitem.ForeignKeys[1]: // refund_items
			values[i] = &sql.NullScanner{S: new(uuid.UUID)}
		default:
...
func (oi *OrderItem) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case orderitem.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field refund_items", values[i])
			} else if value.Valid {
				oi.refund_items = new(uuid.UUID)
				*oi.refund_items = *value.S.(*uuid.UUID)
			}
		default:
