...
func (c *OrderClient) QueryRefunds(o *Order) *RefundQuery {
...
func (c *RefundClient) QueryOrder(r *Refund) *OrderQuery {
	query := (&OrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := r.ID
		step := sqlgraph.NewStep(
...
// QueryItems queries the items edge of a Refund.
func (c *RefundClient) QueryItems(r *Refund) *OrderItemQuery {
	query := (&OrderItemClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := r.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Order, OrderItem, Refund []ent.Hook
	}
	inters struct {
		Order, OrderItem, Refund []ent.Interceptor
	}
)
