...
func (rq *RefundQuery) QueryOrder() *OrderQuery {
...
func (rq *RefundQuery) QueryItems() *OrderItemQuery {
	query := (&OrderItemClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, refund.ItemsTable, refund.ItemsColumn),
		)
		fromU = sqlgraph.SetNeighbors(rq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (rq *RefundQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (rq *RefundQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (rq *RefundQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (rq *RefundQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (rq *RefundQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (rq *RefundQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *RefundQuery) WithOrder(opts ...func(*OrderQuery)) *RefundQuery {
	query := (&OrderClient{config: rq.config}).Query()
...
func (rq *RefundQuery) WithItems(opts ...func(*OrderItemQuery)) *RefundQuery {
	query := (&OrderItemClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RefundQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Refund, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, refund.ForeignKeys...)
	}
	...
	if query := rq.withItems; query != nil {
		if err := rq.loadItems(ctx, query, nodes,
			func(n *Refund) { n.Edges.Items = []*OrderItem{} },
			func(n *Refund, e *OrderItem) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RefundQuery) loadOrder(ctx context.Context, query *OrderQuery, nodes []*Refund, init func(*Refund), assign func(*Refund, *Order)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Refund)
	for i := range nodes {
		if nodes[i].order_refunds == nil {
			continue
		}
		fk := *nodes[i].order_refunds
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_refunds" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *RefundQuery) loadItems(ctx context.Context, query *OrderItemQuery, nodes []*Refund, init func(*Refund), assign func(*Refund, *OrderItem)) error {
	...
	query.Where(predicate.OrderItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(refund.ItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.refund_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "refund_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "refund_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (rq *RefundQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(refund.Table, refund.Columns, sqlgraph.NewFieldSpec(refund.FieldID, field.TypeUUID))
	_spec.From = rq.sql
